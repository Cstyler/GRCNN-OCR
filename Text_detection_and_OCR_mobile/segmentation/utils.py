...
def sparse_accuracy_ignoring_last_label(y_true, y_pred):
    ...
    return K.sum(tf.compat.v1.to_float(legal_labels & K.equal(y_true,
                                                              K.argmax(y_pred,
                                                                       axis=-1)))) / K.sum(
        tf.compat.v1.to_float(legal_labels))
...
def Jaccard(y_true, y_pred):
    ...
    for i in range(0,
                   nb_classes):  # exclude first label (background) and last label (void)
...
class SegModel:
    ...
    def create_seg_model(self, net, n_classes=21, backbone='mobilenetv2',
                         load_weights=False, infer=False,
                         multi_gpu=False, n_channels=3):
        ...
        model = Deeplabv3(weights=None, input_tensor=None, infer=infer,
                          input_shape=self.sz + (n_channels,), classes=n_classes,
        ...
        if backbone == 'xception':
            ...
            scale = 8
        if net == 'original':
            x = Conv2D(n_classes, (1, 1), padding='same', name='conv_upsample')(
                base_model.output)
            size = self.sz[0], self.sz[1]
    ...
    def create_generators(self, crop_shape=False, mode='train', do_ahisteq=True,
                          n_classes=21, horizontal_flip=True,
                          vertical_flip=False, blur=False, with_bg=True, brightness=0.1,
                          rotation=5.0,
    ...
    def train_generator(self, model, train_generator, valid_generator, callbacks,
                        mp=True):
    ...
    def train(self, model, X, y, val_data, tf_board=False, plot_train_process=True):
        h = model.fit(X, y, validation_data=val_data, verbose=1,
                      ...
                      callbacks=self.build_callbacks(tf_board=tf_board,
...
class SegmentationGenerator(Sequence):
    ...
    def __init__(self, folder='/workspace/datasets/', mode='train', n_classes=21,
                 batch_size=1, resize_shape=None,
                 validation_split=.1, seed=7, crop_shape=(640, 320), horizontal_flip=True,
                 blur=0,
                 vertical_flip=0, brightness=0.1, rotation=5.0, zoom=0.1,
                 do_ahisteq=True):
        ...
        if self.crop_shape:
            ...
            self.X = np.zeros((batch_size, resize_shape[1], resize_shape[0], 3),
                              dtype='float32')
            self.SW = np.zeros((batch_size, resize_shape[1] * resize_shape[0]),
                               dtype='float32')
            self.Y = np.zeros((batch_size, resize_shape[1] * resize_shape[0], 1),
                              dtype='float32')
            self.F = np.zeros((batch_size, resize_shape[1] * resize_shape[0], 1),
                              dtype='float32')
            self.F_SW = np.zeros((batch_size, resize_shape[1] * resize_shape[0]),
                                 dtype='float32')
        else:
    ...
    def __getitem__(self, i):
        ...
        for n, (image_path, label_path) in enumerate(
            ...
            valid_pixels = self.F[n][self.Y[
                                         n] != self.n_classes]  # get all pixels (bg and foregroud) that aren't void
            u_classes = np.unique(valid_pixels)
            class_weights = class_weight.compute_class_weight('balanced', u_classes,
                                                              valid_pixels)
            ...
            if len(u_classes):
                class_weights = class_weight.compute_class_weight('balanced', u_classes,
                                                                  filt_y)
                class_weights = {class_id: w for class_id, w in
                                 zip(u_classes, class_weights)}
...
def _random_crop(img, label, crop_shape):
    if (img.shape[0] != label.shape[0]) or (img.shape[1] != label.shape[1]):
        raise Exception('Image and label must have the same dimensions!')

    if (crop_shape[0] < img.shape[1]) and (crop_shape[1] < img.shape[0]):
        x = random.randrange(img.shape[1] - crop_shape[0])
        y = random.randrange(img.shape[0] - crop_shape[1])
        img = img[y:y + crop_shape[1], x:x + crop_shape[0], :]
        label = label[y:y + crop_shape[1], x:x + crop_shape[0]]
        return img, label
    else:
        img = cv2.resize(img, crop_shape)
