...
from tensorflow.keras.layers import ReLU, Softmax, Add, AveragePooling2D, \
                                                  BatchNormalization, \
    Concatenate, Conv2D, DepthwiseConv2D, Dropout, Input, Lambda, Reshape, ZeroPadding2D
...
def _xception_block(inputs, depth_list, prefix, skip_connection_type, stride,
    ...
    if skip_connection_type == 'conv':
        ...
        raise ValueError(f"Wrong skip_connection_type: {skip_connection_type}")
    if return_skip:
        return outputs, skip
    else:
        return outputs
...
def Deeplabv3(weights='pascal_voc', input_tensor=None, infer=False,
    ...
    size = int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))
    method = tf.image.ResizeMethod.BILINEAR
    b4 = Lambda(lambda x: tf.image.resize(x, size, method))(b4)
    ...
    if backbone == 'xception':
        ...
        b0 = Lambda(lambda x: tf.image.resize(x, size, method))(b0)
        x = Concatenate()([b4, b0])
