...
def get_fg_mask(bg_color, samples, value_threshold=25, sat_threshold=20):
...
def get_palette(samples, value_threshold, sat_threshold, num_colors,
                return_mask=False,
                kmeans_iter=40, quiet=True):
...
def apply_palette(img, palette, value_threshold, sat_threshold, quiet=True):
...
def enhance_noteshrink(img, sample_fraction, value_threshold=25, sat_threshold=20,
                       num_colors=8):
    samples = sample_pixels(img, sample_fraction)
    palette = get_palette(samples, value_threshold, sat_threshold, num_colors)

    labels = apply_palette(img, palette, value_threshold, sat_threshold)
    return labels
