...
class DocScanner(object):
    ...
    def get_corners(self, img):
        ...
        if lines is not None:
            ...
            (contours, hierarchy) = cv2.findContours(horizontal_lines_canvas,
                                                     cv2.RETR_EXTERNAL,
                                                     cv2.CHAIN_APPROX_NONE)
            contours = sorted(contours, key=lambda c: cv2.arcLength(c, True),
                              reverse=True)[:2]
            horizontal_lines_canvas = np.zeros(img.shape, dtype=np.uint8)
            ...
            (contours, hierarchy) = cv2.findContours(vertical_lines_canvas,
                                                     cv2.RETR_EXTERNAL,
                                                     cv2.CHAIN_APPROX_NONE)
            contours = sorted(contours, key=lambda c: cv2.arcLength(c, True),
                              reverse=True)[:2]
    ...
    def is_valid_contour(self, cnt, IM_WIDTH, IM_HEIGHT):
        ...
        return (len(cnt) == 4 and cv2.contourArea(
            cnt) > IM_WIDTH * IM_HEIGHT * self.MIN_QUAD_AREA_RATIO
