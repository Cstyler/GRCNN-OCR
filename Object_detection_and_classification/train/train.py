...
import torchvision
from dataset import ProductsDataset
import transforms
import utils
...
def main(args):
    ...
    if not args.prototype:
        from torchvision.models.efficientnet import model_urls, load_state_dict_from_url
        ...
        for m in model.modules():
            if isinstance(m, nn.Linear):
                state_dict["classifier.1.weight"] = m.weight
                state_dict["classifier.1.bias"] = m.bias
                break
        model.load_state_dict(state_dict)
    else:
        model = prototype.models.__dict__[args.model](
    ...
    if args.resume:
        ...
        if not args.test_only and args.resume_optimizer:
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--data-path", default="../dataset", type=str, help="dataset path"
    )
    ...
    parser.add_argument("--resume_optimizer", default=True, type=bool)
    parser.add_argument(
